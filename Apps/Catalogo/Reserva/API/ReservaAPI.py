...
class ReservaModelViewSet(ModelViewSet):
    ...
    def list(self, request):
        serializer = ReservaSerializer(Reserva.objects.all(), many=True)
    ...
    def retrieve(self, request, pk= int):
        try:
            serch = Reserva.objects.get(pk=pk)
            serializer = ReservaSerializer(serch)
            data = ResponseData(
                Success=True,
                Status=status.HTTP_200_OK,
    ...
    def update(self, request, pk= int, **kwargs):
        ...
        try:
            certific = Reserva.objects.get(pk=pk)
            serializer = ReservaSerializer(instance=certific, data= request.data, partial=partial)
            serializer.is_valid(raise_exception=True)
            serializer.save()
    ...
    def destroy(self, request, pk: int):
        try:
            cert = Reserva.objects.get(pk=pk)
